...
byte-identical transcript differing only in the final answer. **The 2026-08-10 `decision_quality`
defect is closed and `565a92c` did not over-correct.** Zero quota text in the output.

**🔴 It does not close 4.3, and it says nothing about the empty-generation fault.** The pair tests
the Evaluator in isolation, not the graph; the 75s conduct-loop re-run is still the only thing
blocking the story. And `llm_schema_failure` is a log record a green `-q` run discards. See
...
| 5 Coach | 🟢🔴 **ALL FOUR STORIES BUILT AND THE GATE WAS SAT LIVE 2026-08-12. DEMO-READY BY KARTHIK'S CALL; THE OPEN WORK IS AGENT QUALITY, NOT CORRECTNESS.** A full Senior PM interview ran on the deployed stack end to end and produced a coaching report. **Everything structural passed**: the graph exited through the Coach, three rows were written, **all three anchor quotes are verbatim candidate text** (checked against `transcript_turns`), a mid-interview Evaluator failure **degraded without ending the session** — 2026-08-11's fix observed in **production** — and **migration `0006` is confirmed end to end**, the panel filling in over Realtime with **no refresh**, which `probe_realtime.mjs` structurally cannot show since it exercises `agent_events`. The UI was fixed the same day: the closing card stopped claiming scoring "is not part of this build yet", and **the report moved from the narrow evaluation rail to the wide middle column** (`194 passed`). 🔴 **Three defects, one a rule violation: every `stronger_version` invents dollar figures** (`$10` CPM, `$20,000` per 1% share, `$12 × 60 = $720`, wrong on units), breaking CLAUDE.md § Design's fake-round-numbers rule — **cause traced to 5.2's own budget fix**, which stopped sending `supporting_facts`; **the three improvements collapse into one idea** (all three drills "compare CPMs"), which is the padding risk "three, always" accepted; and **2 of 4 probes attributed CASE-WORLD text to the candidate**, traced to `app/cases/youtube.json` lines 40 and 43. 🔴 **The Evaluator returned `4` for all 17 dimension scores across 4 answers**, so nothing was unevidenced and **the `gap` path has still never run**. **Nothing was fixed — prompt and quality calls are Karthik's under the standing rule.** 5.3's single-call assertion and `falsify_coach_single_call.py` are **committed unverified and labelled so**; the full live suite from the two migrations was **deliberately skipped**, recorded as a decision, not an oversight. See § Decisions 2026-08-12 and § Next session. Prior note: **5.1, 5.2 AND 5.3 BUILT AND OFFLINE-GREEN 2026-08-11. 5.4 IS NEXT AND COSTS NO LLM BUDGET.** `coach_reports` table applied and its constraints falsified in Postgres; `app/agents/coach.py` with `verify_anchors` **observed catching both an invented quote AND a false coverage gap**; the graph now ends THROUGH the Coach (`decide_next` "exit" -> `coach_report` -> `END`), degrading on failure like the Evaluator. **Both golden cases observed passing live, but in SEPARATE runs** — one clean pair run is owed (~7k). The day ended on the daily cap at `Used 197286/200000`, classified quota. **The budget was MEASURED and the spec's projection was wrong by ~2,000 tokens** (8,328 vs 8,000), fixed by summarising the world and capping quotes at 2 per dimension. **The em-dash trap fired a third time and prompting failed again** — resolved by aligning the new suite to the project's established four-character standard rather than by prompting harder. See § Decisions 2026-08-11. Prior note: **SPECCED 2026-08-11, nothing built.** Four findings shape it. **There is no `coach_report` table** — `state.py:70` has the field, the schema has no home for it, so 5.1 writes the migration BEFORE the agent (4.3's problem inverted: there the DDL had already decided the shape, here nothing has). **ARCHITECTURE's "whole transcript in one 1M-context call" does not exist on this stack** — 8,000 TPM against a transcript measured at 10,274 — but the Coach has an exit the Evaluator did not: it reads `answer_evaluations` (score + verbatim quote + `0004`'s `reasoning`), so it consumes **judgement, not raw material**, and one call becomes possible. Budget **PROJECTED at ~6,290/8,000 and explicitly NOT measured** — 5.1 owes the real number. **The Coach must survive partial or zero evaluations**, newly true as of the 2026-08-11 degrade fix, so it returns **up to** three improvements and may return fewer; forcing three would invent two. **The anchor quote must be one of the stored `evidence_quote` values**, which makes every anchor verifiable byte-for-byte without the Coach seeing the transcript. `fast`, per the 2026-08-02 calibration, superseding PRD §6's `deep` | [PHASE-5-SPEC.md](specs/PHASE-5-SPEC.md) | — |
...
| Evaluator | ✅ [AGENT-EVALUATOR-SPEC.md](specs/agents/AGENT-EVALUATOR-SPEC.md) — written 2026-08-09 **after** the agent, deliberately: PHASE-4-SPEC carried the contract through 4.1-4.3, so an up-front copy would have been a duplicate to keep in sync rather than a design | 7 written blind 2026-08-08, **reusing the planner's case worlds by pointer** except the one documented exception (Karthik's real 2026-08-07 interview, which has nothing to point at). **2 of 7 smoked live 2026-08-09.** `apm_consumer` green on both roles; `sparse_world_framework_narration` green on `deep`, red on `fast` on a **rubric definition disagreement, not a malformed output** — see § Decisions 2026-08-09 #8. 🔴 **fixture 1's ground truth is KNOWN WRONG** and is 4.3's to fix: it describes the whole interview while the call scores one answer, and its `not_assessed` was inherited from the Interviewer's probe ledger | **2026-08-09 — first prompt.** Four ordered steps, decline-to-score made a first-class result, and `framework_narration` strengthened after `fast` returned `False` on an answer that walks RICE's letters out loud. **Runs on `fast`** (measured, not inherited) |
| Coach | ⬜ **not written** — the agent shipped first, like the Evaluator's did. PHASE-5-SPEC carries the contract | 2 written 2026-08-11 (`tests/golden/coach/`). **Both have passed live, in SEPARATE runs, never together.** One clean pair run owed, ~7k | **2026-08-11 — first prompt, never edited.** 🔴 **Its first real output invented dollar figures** in every `stronger_version`, breaking CLAUDE.md § Design's fake-round-numbers rule. Cause is dated and known: 5.2 stopped sending `supporting_facts` to fit the ceiling, so it holds no company facts. Fix recommended as a deterministic guard, **not applied** — Karthik's call. See § Decisions 2026-08-12. Runs on `fast` |
...
## 🟢 SESSION 21. PHASE 5 RUNS END TO END AND IS DEMO-READY. EVERY REMAINING GAP IS LISTED
## BELOW, AND NONE OF THEM BLOCK A RECORDING.

**Karthik's call, 2026-08-12: this is enough for the demo he wanted to record.** Phase 5 is not
being pushed further for polish. The gaps below are real and written down so they are not
rediscovered by accident; they are not a queue that must be cleared before anything else happens.

**Everything is pushed. `git status` is clean.** The deployed stack ran a full interview today and
produced a coaching report.

**Free checks, ~3 min, no LLM:**

```
cd backend && .venv/Scripts/python.exe -m pytest tests -q -m "not live"   # expect 507 passed, 118 deselected
cd frontend && npm test -- --run                                          # expect 194 passed, 19 files
```

🔴 **`118` deselected, not 117.** The new one is `test_coach_report_produces_exactly_one_llm_call_per_session`.
**Deselected is not passed** — it has never run.

### 🔴 THE GAPS, in the order I would spend budget on them

**1. Two live items, cheap, and they close story 5.3's last acceptance box.**

```
backend/.venv/Scripts/python.exe backend/scripts/falsify_coach_single_call.py   # ~2 coach calls
cd backend && .venv/Scripts/python.exe -m pytest tests/golden/coach -q -m live  # ~7k, the pair
```

Both coach golden cases have passed live but **in separate runs, never together**. The
falsification script has **never executed**; until it does, 5.3's single-call assertion is unproven
as a gate. Read `falsify_coach_single_call.py`'s exit code: **2 means the assertion is vacuous.**

**2. The new live coach assertion has never run.** `test_coach_report_produces_exactly_one_llm_call_per_session`
in `tests/test_conduct_loop.py` drives a full interview to the exit boundary — **~10 `fast` calls,
the most expensive test in the file.** Worth a fresh day, not the tail of a spent one.

**3. 🔴 THE COACH INVENTS DOLLAR FIGURES. This is the biggest open defect and it is a rule
violation, not a taste call.** Every `stronger_version` in the live report carried fabricated
money (`$10` CPM, `$8`, `$20,000` per 1% of share, `$12 × 60 = $720` which is also wrong on units).
CLAUDE.md § Design's **fake-round-numbers rule binds agent prompts**, and this breaks it.

**The cause is known and dated**: story 5.2 stopped sending `supporting_facts` to fit the 8,000
ceiling, so the Coach writes about ad economics holding no company facts and fills the hole.

**Recommended fix is a deterministic guard, not a prompt edit** — reject a `stronger_version`
carrying a currency or percentage token absent from the source material, then regenerate. Same
shape as `verify_anchors`, zero tokens to test. **Prompting has lost to this class of problem three
times in this project and a guard has won every time.** Not applied: prompt and quality calls are
Karthik's.

**4. The three improvements collapse into one idea.** All three drills in the live report were
"compare CPMs in a spreadsheet". This is the padding risk the 2026-08-11 "three, always" decision
knowingly accepted, arriving as redundancy rather than filler. **Karthik's call whether that
decision survives.**

**5. The `gap` path has never run.** The Evaluator scored every dimension, so
`unevidenced_dimensions` was empty and all three improvements were `moment`s. Nothing is known
about how a `gap` reads in a real report.

**6. 🔴 The Evaluator returned `4` for ALL 17 dimension scores across 4 answers.** No 3, no 5, on a
0-4 scale. A rubric emitting a constant is not discriminating. Second time this has come up.

**7. 2 of 4 probes attributed CASE-WORLD text to the candidate.** Traced to
`app/cases/youtube.json` line 43 (an `option`) and line 40 (the `tension`): the answer matched half
an option and `generate_probe` attributed the whole option back. New mechanism in the 2026-08-08
misattribution family. See § Decisions 2026-08-12.

**8. The full live suite is STILL owed from migrations `0005`/`0006`**, and was deliberately not
run. CLAUDE.md's portfolio calibration says the full live suite is **not a gate**, and the live sit
covered both migrations in the real path — `0005` by writing a report, `0006` by the panel
arriving with no refresh. **This is a recorded, deliberate skip, not an oversight.** What it does
not cover is the story-0.5 shape: a migration breaking an assertion no human path touches.

**9. One transient never explained.** The Evaluator errored mid-interview at 06:14:32 and the
session correctly degraded. **It is recorded as UNCLASSIFIED** — `agent_events` stores no error
text and the UI's "Show details" string was never captured. 63s between `started` and `error` fits
both a 429 and the known `failed_generation=''` fault. **If this recurs live, capture that text
first.**

### 🟢 What session 20 delivered

- **The live gate was sat.** Full Senior PM interview on the deployed stack, YouTube world.
  Anchors verbatim, degrade path proven in production, `0006` confirmed end to end.
- **The closing card stopped lying** ("Feedback and scoring are not part of this build yet") and
  **the coaching report moved to the wide middle column**, where prose is readable. `194 passed`.
- **Three stale comments fixed**, including one claiming the Realtime INSERT handler could never
  fire — false since `0006`, and it would have sent a future session hunting a fixed bug.
- **Story 5.3's single-call assertion and falsification script written**, committed unverified and
  labelled as such.

---

## Superseded — session 20's opening handoff, kept for the record
...
**2. ✅ DONE 2026-08-11 — DO NOT RE-RUN. THE FOLLOW-UP PAIR PASSED LIVE: `2 passed, 43 deselected,
131.41s`.** Both halves, `defends` ≥ 3 and control `abandons` ≤ 2, zero quota text. The 2026-08-10
`decision_quality` defect is closed and the fix did not over-correct. § Decisions 2026-08-11 has
the full reading. The original brief is kept below for the record:
...
..                                                                       [100%]
2 passed, 43 deselected, 2 warnings in 131.41s (0:02:11)
```

**Read as one result, which is the only way this pair means anything.** Same world
(`pm_b2b_world`), same level (PM), turns 0-2 byte-identical; only the final candidate answer
differs.

- `followup_defends_decision` passed its assertions `decision_quality != 1` **and** `>= 3`.
- `followup_abandons_decision` passed its assertion `decision_quality <= 2`.

So the two halves landed **at least one point apart on the same transcript**, which is the property
the pair exists to observe. The exact scores are not recoverable from a green `-q` run and were not
worth a second ~14,000 tokens to print — the bounds are what the assertions gate on.

**Why this is not vacuous.** The failure shape this project has shipped three times is an assertion
that passes because the model scores *everything* well. The control half forbids that: a high score
on `abandons` fails the run. Both halves passing means the rubric still discriminates after
`565a92c`. Assertion text read at `tests/golden/evaluator/cases.py:235-292` before believing the
green.

**Quota classified, per the standing rule.** Zero occurrences of `tokens per day` or `tokens per
minute` in the output, and zero failures, so nothing was masked.

**🔴 What this does NOT close.** Two things carried forward unchanged:

1. **4.3 is still open.** This pair exercises the Evaluator in isolation, not the graph. The
   `test_conduct_loop.py` + `test_transcript.py` re-run at 75s pacing is still the only thing
   blocking the story.
...
**🟢 2026-08-11 (session 19) · STORY 4.3 IS CLOSED. THE 75s RUN PASSED EVERY TEST IN BOTH FILES —
ACROSS TWO RUNS, NOT ONE, AND THE DIFFERENCE MATTERS.**

```
1 failed, 9 passed, 32 deselected, 34 warnings in 2468.42s (0:41:08)
```

then the single failure, re-run alone:

```
1 passed, 35 deselected, 7 warnings in 237.54s (0:03:57)
```

**Classified before concluding, per the standing rule.** Grepped the 41-minute log:

```
tokens per day / tokens per minute / RateLimitError / 429 :  0
AssertionError                                            :  0
llm_schema_failure                                        :  0
```

**So: zero quota, zero assertion failures, and — notable — zero schema failures.** The Evaluator's
`failed_generation=''` fault did not fire once in 41 minutes at 75s pacing. **That is one clean
sample, not a fix**; 2026-08-10 #9 proved the fault real in a human-paced interview. It stays open.

**🔴 The one failure, and the honest account of it.** `psycopg.OperationalError: server closed the
connection unexpectedly`, raised inside `checkpointer.aget_tuple` — a pure DB read with no LLM
involved — on `test_await_candidate_does_not_redo_the_clarification_call_amid_a_probe_loop`. It
struck at `test_conduct_loop.py:952`, **after both of that test's assertions had already passed**
(`calls_before_clarify == 2`, `calls_after_clarify == 3`). The `agent=` labels in the captured log
confirm the interviewer/evaluator split was exactly as designed.

**🔴 The obvious explanation was tested and FALSIFIED, and this is the part worth keeping.** The
natural story is that 75s pacing leaves the pooled connection idle long enough for Supabase's
session pooler to reap it — which would make it a harness artifact, since production opens a fresh
connection per HTTP request (story 0.7's stateless-HTTP proof). A zero-token repro held an
`AsyncPostgresSaver` connection idle for 80s and then read through it:

```
[   0.2s] read 1 OK (connection is alive)
[   0.2s] sleeping 80.0s, holding the connection idle...
[  80.2s] read 2 OK (connection survived the idle gap)
```

**The connection survived. The hypothesis is wrong.** It was one edit away from being written into
this file as fact.

**So the cause is UNDETERMINED.** It is recorded as transient because **it does not reproduce** —
the same test passed alone four minutes later — **not because it is understood.** Reopening
condition: if `OperationalError` appears again in any live run, it is a real defect and the pooler
configuration is the first place to look, not the last.

**Why 4.3 closes on this.** Every assertion in both files has been observed passing, nothing in the
product failed, and the assertion 4.3 actually broke —
...
Found while gathering the Coach's token measurement for Phase 5, not by a failing test — which is
the whole problem.

`evaluate_answer` ships **`max_tokens=4096`** (`app/agents/evaluator.py:413`). Both
`tests/test_evaluator_budget.py` (`_MAX_TOKENS = 2048`) and `evaluator.py:381`'s own docstring still
say **2048**. **`max_tokens` is part of the REQUEST size on Groq, not merely a cap on the reply** —
`app/llm.py:367` says so in capitals — so the budget test understates every request by exactly
2,048 tokens.

Re-measured offline at zero token cost, reusing the budget test's own apparatus
(`_FULL_PRIOR_SCORES`, `_first_question`, `_last_answer`) so the arithmetic is identical:

```
fixture                                            body   @2048   @4096    verdict@4096
karthik_live_airbnb_senior_pm                      3281    5329    7377       623 spare
apm_consumer_world_full_coverage                   3339    5387    7435       565 spare
pm_b2b_world_narrow_coverage                       3124    5172    7220       780 spare
senior_pm_platform_world_level_pair_pm             3321    5369    7417       583 spare
senior_pm_platform_world_level_pair_gpm            3322    5370    7418       582 spare
gpm_portfolio_world_extreme_narrow                 3059    5107    7155       845 spare
sparse_world_framework_narration                   2838    4886    6934      1066 spare
followup_defends_decision                          3208    5256    7304       696 spare
followup_abandons_decision                         3152    5200    7248       752 spare
```

**Nothing is over the ceiling today, so this is not a live outage.** What it is: **headroom fell
from ~2,613 tokens to 565 on the worst fixture — 93% of the 8,000 TPM ceiling — and the test whose
entire job is to notice that stayed green.** It would keep passing well past 8,000. That is the
vacuous-green shape, and it is the reason the test was written in the first place.

**It also reframes the two-call retry.** DEV-STATE 2026-08-10 records `max_tokens` 2048 → 4096 as
having "moved it, did not close it", with every evaluation now costing two calls. At 7,435 tokens a
validate-retry **inside the same minute** requests ~14,870 against an 8,000/min ceiling, so the
retry cannot succeed on pacing grounds alone. **This does not prove the cause of the
`failed_generation=''` fault** — do not record it as one — but it means the bump bought output room
by spending nearly all the per-minute headroom, and no one priced that trade.

**🔴 NOT FIXED IN THIS SESSION, DELIBERATELY.** The 4.3 live conduct-loop run was executing against
`app/agents/evaluator.py` at the time. Editing the file mid-run would have validated a tree that no
longer existed and owed a second ~30,000-token run — the same ordering trap that put the defect fix
before the run rather than after it.

**The fix when it happens is not "change 2048 to 4096".** That re-arms the identical drift. Extract
one module-level constant in `evaluator.py`, have the call site AND the test both read it, so the
two cannot disagree again. Then re-assert the ceiling against the real number.
...
**🟢 10. STORY 4.4 IS DONE. THE SCORECARD EXISTS, AND IT REFUSES TO PRINT AN AVERAGE IT CANNOT
JUSTIFY.** `82c2392`. Zero LLM budget: the whole story is frontend, and everything it needed already
existed (`answer_evaluations` has been in the realtime publication since migration 0001 and has
carried an own-session `select` policy since 0002).

```
frontend    170 passed, 17 files   (was 141 passed, 15 files)
tsc -b      exit 0        oxlint clean
```

**The suppression rule is FALSIFIED, not trusted.** Relaxing the completeness condition to
`if (!blind)` turns the suite red — **and it fails on the POSITIVE coverage assertion**, not the
absence check. That ordering is what stops the test passing against a blank component, which is the
vacuous shape this project has shipped twice.

Design calls, all made by the orchestrator before delegating: latest row per dimension wins
(mirroring `_prior_scores`, so both ends mean the same thing by "the score") · `not_assessed` is a
dashed unfilled track with no numeral, visually distinct from a 1 · **no overall score at all when
any dimension is unassessed**, coverage instead.

**Three subagent judgment calls, flagged rather than buried, all kept:** `revealed` HIDES the blind
toggle rather than disabling it (a dead control reads as a bug) with the requested state persisting
underneath · the `empty` state does NOT render five "Not assessed" rows, which would make it
identical to a scored-nothing `ready` · `App.tsx` and `App.test.tsx` were touched outside the brief
because the column needs the hoisted `sessionId` and the App test would otherwise open a real
websocket.

**🔴 11. NO npm COMMAND TYPECHECKS A TEST FILE. Found 2026-08-09, recorded and NOT fixed.**

`frontend/tsconfig.json` references only `tsconfig.app.json` and `tsconfig.node.json`.
**`tsconfig.vitest.json` is referenced by nothing**, so `npm run build` and every other script skip
test files entirely. Running it directly:

```
npx tsc -p tsconfig.vitest.json --noEmit   ->  9 errors, ALL in src/components/UploadSurface.test.tsx
```

All nine are the same vitest `Mock<Procedure | Constructable>` assignability pattern, and that file
is untouched by story 4.4, so they are pre-existing by construction. **Deliberately not fixed
inside 4.4** — wiring the reference in would surface nine unrelated errors and turn a finished story
into a refactor. **Phase 7 (polish and hardening) work**, and worth doing then: a typecheck that
cannot see test files is a verification hole, not a preference.
