...
### 2.3 The Case Architect agent and `generate_case_world` — 🟡 AGENT DONE 2026-08-04, NODE UNPROVEN
...
- [x] **`case_world` is written exactly once and is immutable thereafter** — ARCHITECTURE §2. **DONE 2026-08-04, both halves.** Immutability across `plan_interview` is asserted and passes live. The write-once half was enforced by NOTHING (0001's own comment said so); `0003_case_world_write_once.sql` adds `unique (session_id)` and `tests/test_case_world.py` **watches a second write be rejected** with `UniqueViolation` — 3 passed, with a positive control and a per-session control so it cannot pass vacuously
...
### 2.6 The Planner agent and `plan_interview` — 🟡 AGENT DONE 2026-08-04, NODE UNPROVEN
